...
class Psda extends BaseController
{
    ...
    public function update_anggota()
    {
        ...
        // 🚀 tambahan pindah ke alumni kalau status Alumni/Demisioner
        $status = $this->request->getVar('keanggotaan');
        if ($status === 'Alumni' || $status === 'Demisioner') {
            $this->alumniModel->insert([
                'npm'           => $this->request->getVar('npm'),
                'nomor_anggota' => $this->request->getVar('nomor_anggota'),
                'nama_alumni'  => $this->request->getVar('nama'),
                'email_alumni'   => $this->request->getVar('email'),
                'no_hp_alumni'   => $this->request->getVar('no_handphone'),
                'jenis_kelamin' => $this->request->getVar('jenis_kelamin'),
                'jurusan'       => $this->request->getVar('jurusan'),
                'fakultas'      => $this->request->getVar('fakultas'),
                'status_alumni'        => $status,
                'slug_alumni'   => url_title($this->request->getVar('nama'), '-', true),
            ]);

            // optional: hapus dari data_anggota
            $this->data_anggota
                ->where('nomor_anggota', $this->request->getVar('nomor_anggota'))
                ->delete();

            session()->setFlashdata('success', 'Data berhasil dipindahkan ke Alumni');
        } else{

            session()->setFlashdata('success', 'Data anggota berhasil diudpate');
        }
